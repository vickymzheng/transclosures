...
// This processing file communicates with the overlapper
...
void print_nodes(const Collection& c) {
  for (const int x : c) {
    std::cout << x << std::endl; 
  }
}
...
int trans_closures_stream(std::unordered_map<int, std::vector<int> >& trans_adjacency_list, 
                        std::set<int>& irreducible_nodes, std::set<int>& all_nodes, std::unordered_map<int, 
                        int>&  component_mapping, std::unordered_map<int, std::set<int> >& component_members,
  ...
  while(std::cin >> curr_edge.source) {
    if (curr_edge.source == -1) {
      ...
      print_nodes(new_removed_nodes);
      std::cout << "-1" << std::endl; 
      num_removed_nodes+=new_removed_nodes.size();
      curr_read = -1; 
      ++num_reads;
...
int main(int argc, char* argv[]) {
  ...
  stat_file << "Output file is " << output_file_prefix << "." << std::endl;
  stat_file << "Component size threshold is set as " << component_size_threshold << "." << std::endl;
  ...
  int max_number_of_reads = trans_closures_stream(trans_adjacency_list, irreducible_nodes, all_nodes, component_mapping,component_members, output_file_prefix);
